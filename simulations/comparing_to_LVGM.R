...
nvec <- floor(exp(log(1000)/ratio))
hvec <- c(1,2,3) # number of latent variables
m = 1
lambda_2 <- c(4) # gamma for eglatent
num_iter <- 50
plot_result <- FALSE
...
F1score_LVGM_cv<- list()
rk_LVGM_cv <- list()
F1score_LVGM_oracle<- list()
rk_LVGM_oracle <- list()
...
for (n_iter in 1:length(nvec)){
  ...
  for (h_iter in 1:length(hvec)){
    ...
    F1score_LVGM_cv_vec <- c()
    rk_LVGM_cv_vec <- c()
    F1score_LVGM_oracle_vec <- c()
    rk_LVGM_oracle_vec <- c()
    ...
    for (iter in 1:num_iter){
      ...
      # applying approach using latent variable Gaussian graphical models
      DataX_train_Gaussian <- c()
      DataX_valid_Gaussian <- c()
      for (i in 1:d){
      DataX_train_Gaussian<- cbind(DataX_train_Gaussian,qnorm(graphicalExtremes:::unif((output$data_train)[,i])))
      DataX_valid_Gaussian <- cbind(DataX_valid_Gaussian,qnorm(graphicalExtremes:::unif((output$data_test)[,i])))
      }
      result_LVGM <- fit_LVGM(DataX_train_Gaussian,DataX_valid_Gaussian)
      F1score_LVGM_cv_vec <- append(F1score_LVGM_cv_vec,result_LVGM$F1_score_opt_valid)
      rk_LVGM_cv_vec <- append(rk_LVGM_cv_vec,result_LVGM$h_opt_valid)
      F1score_LVGM_oracle_vec <- append(F1score_LVGM_oracle_vec,result_LVGM$F1_score_opt_oracle)
      rk_LVGM_oracle_vec <- append(rk_LVGM_oracle_vec,result_LVGM$h_opt_oracle)
    ...
    F1score_LVGM_cv <- append(F1score_LVGM_cv,list(F1score_LVGM_cv_vec))
    rk_LVGM_cv <- append(rk_LVGM_cv,list(rk_LVGM_cv_vec))
    F1score_LVGM_oracle <- append(F1score_LVGM_oracle,list(F1score_LVGM_oracle_vec))
    rk_LVGM_oracle <- append(rk_LVGM_oracle,list(rk_LVGM_oracle_vec))
...
data<- tibble(F1score_cv = F1score_cv,  F1score_oracle =  F1score_oracle, rk_oracle = rk_oracle, rk_cv = rk_cv, likelihood_cv = likelihood_cv, F1score_eglearn_cv = F1score_eglearn_cv, F1score_eglearn_oracle = F1score_eglearn_oracle, likelihood_eglearn_cv = likelihood_eglearn_cv, F1score_LVGM_cv= F1score_LVGM_cv, F1score_LVGM_oracle =  F1score_LVGM_oracle,rk_LVGM_cv = rk_LVGM_cv,rk_LVGM_oracle=rk_LVGM_oracle)
